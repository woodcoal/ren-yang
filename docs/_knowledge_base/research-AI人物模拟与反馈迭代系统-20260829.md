...
9. 前期是否应使用 SQLite 降低部署成本，并保留向 PostgreSQL 升级的路径？
10. OpenViking 如何成为可选能力，关闭后哪些能力保留、哪些能力降级？
11. 核心系统完成后，哪些相邻业务最值得扩展？
...
- 文本和图片生成属于可持续数十秒到数分钟、需要重试和恢复的任务，不能绑定普通 HTTP 请求生命周期。API 只创建持久化任务并返回 `runId`；独立 Node Worker 从业务数据库领取任务、写入进度和结果。
- Nitro Tasks 与 Database 当前仍是实验能力，不应作为核心业务任务的唯一可靠性基础。MVP 通过成熟的数据库驱动或 ORM 访问 SQLite，后续可迁移至 PostgreSQL；任务状态不能只保存在进程内存、Nitro KV 或 Serverless 平台的临时状态中。
...
1. Mem0 不再进入技术方案；OpenViking 作为可选的语义检索适配器，不是系统启动和运行的前置条件。
2. 关系型业务库仍是人物版本、参数、任务、图文块、反馈和历史记录的唯一事实源；MVP 默认 SQLite，达到并发或部署阈值后迁移至 PostgreSQL。
3. 推荐技术组合调整为“Nuxt 4 全栈应用 + 独立 Node Worker + SQLite + 可选 OpenViking + 对象存储”，不增加 .NET 或 Python 中间层。
...
运行历史必须能查看：原始任务、人物版本、格式模板版本、参数快照、模型与提示版本、上下文提供器及检索证据、每个块的历次尝试、最终组装结果、耗时、成本、错误和反馈。重新生成形成新的 `BlockAttempt`，不能覆盖旧结果。
...
模型供应商、上下文检索、图片生成和对象存储放在内部接缝之后。上下文检索模块只暴露同步资料、检索证据和健康检查等少量接口；OpenViking 适配器不能把其 136 个 HTTP 操作泄漏给业务模块。

### OpenViking 可选集成

| 数据 | 存放位置 | 原因 |
|---|---|---|
| 人物、版本、参数、任务、状态、成本、反馈 | SQLite；后续 PostgreSQL | 需要事务、约束、关联查询和业务审计 |
| 资料元数据、解析正文和文本块 | 业务数据库 / 对象存储 | OpenViking 关闭后仍必须可以生成和重建索引 |
| 语义索引与分层上下文 | OpenViking Resource，可选 | 启用时提供解析、分层内容和语义检索 |
| 已确认的长期反馈结论 | 业务数据库；可选同步至 OpenViking | 业务库是事实源，只有已发布修订可参与后续检索 |
| 原始上传、生成图片、导出文件 | 对象存储 | 二进制产物不应塞入业务表 |
| 单次运行实际使用的证据快照 | 业务数据库 / 对象存储 | 保证检索源变化后仍可复现 |

使用配置 `CONTEXT_PROVIDER=local|openviking` 选择适配器，MVP 只支持系统级切换，不增加人物级和单次运行级开关：

| 能力 | `local` | `openviking` |
|---|---|---|
| 人物资料与正文读取 | 支持 | 支持 |
| 基础关键词检索 | SQLite FTS5 | 支持 |
| 语义检索、分层上下文 | 不支持 | 支持 |
| 已确认反馈参与检索 | 支持基础筛选 | 支持语义召回 |
| 系统主流程 | 完整可运行 | 完整可运行 |

本地适配器使用业务库中的 `SourceChunk` 与 SQLite FTS5 返回基础证据；OpenViking 适配器同步相同资料并调用 `find`。切换适配器时通过后台任务重建目标索引，不改人物版本和历史记录；每次运行保存实际使用的适配器、证据内容与哈希。关闭 OpenViking 只代表检索效果降级，不删除其中已有资源，也不阻断兴趣判断和图文生成。

启用 OpenViking 时，建议人物目录为 `viking://resources/characters/{personaId}/canon/`、`examples/`、`published/`。发布人物版本时同步 `published/` 并保存 URI、内容哈希和同步状态；运行时对已知目录优先调用 `find`，不默认创建 OpenViking 会话，也不把未确认输出自动提交为记忆。

### 最小数据对象

- `Persona`：人物逻辑标识；
- `PersonaVersion`：不可变人物快照、状态与父版本；
- `CharacterSource`：原著、设定、对白、参考图、原始位置和内容哈希；
- `SourceChunk`：资料解析后的文本块和本地检索字段；
- `Evidence`：资料、样例、反馈或运行结果中的证据；
- `FormatTemplate`：允许的块类型、渲染载体和验证规则；
- `ParameterProfile`：分层参数及版本；
- `FeedbackEvent`：原始反馈，不可覆盖；
- `GenerationRun`：人物、模板、参数快照、模型、提示、上下文适配器、证据、状态和成本；
...
MVP 使用一个 Nuxt 4 全栈应用、一个同仓库独立 Node Worker、SQLite、可选 OpenViking 和兼容 S3 的对象存储即可。OpenViking 启用时不再引入额外向量库；关闭时只使用 FTS5，不为追求等价语义效果再安装另一套向量系统。耗时生成先用数据库任务表和单 Worker，出现明确吞吐瓶颈后迁移 PostgreSQL 或引入专用队列。

### 技术栈结论

| 方案 | 优点 | 主要代价 | 结论 |
|---|---|---|---|
| Nuxt 4 + SQLite + Node Worker | 零数据库服务；单文件备份；管理端与 API 共享类型 | 写并发和横向扩展有限 | 本地与早期 MVP 首选 |
| Nuxt 4 + PostgreSQL + Node Worker | 并发领取任务、远程访问、备份与扩展更成熟 | 增加数据库部署与运维 | 生产升级目标 |
...
直接使用 Nuxt 与 SQLite 是更小且完整的 MVP 方案。此前推荐 .NET 与 PostgreSQL 是基于长期业务系统可靠性的保守选择，不是当前阶段的必要成本；先使用 Nuxt 4 + Nitro API，并把耗时生成放入独立 Node Worker。Python 只在后续训练、离线评测或本地图像算法确有需要时作为独立工作进程加入。

### 数据库升级策略

SQLite 的“更快”主要是开发和部署更快，不代表查询一定优于 PostgreSQL。当前单机、少用户、单 Worker 场景适合 SQLite，并应启用 WAL、外键约束和事务。PostgreSQL 是升级目标，不是 SQLite 的在线备用库；MVP 不做双写、实时同步或故障自动切换。

应用使用 `DATABASE_PROVIDER=sqlite|postgresql` 选择唯一生效的数据库适配器，默认 `sqlite`。这保留了切换能力，但任一时刻只有一个事实源；业务模块不能直接依赖具体数据库方言。

为保留迁移能力，数据库访问集中在内部模块，迁移文件避免依赖 SQLite 专属行为或 PostgreSQL 专属 `JSONB`、数组和锁语法。达到以下任一条件时迁移 PostgreSQL：

- Web 或 Worker 需要多实例部署；
- 多 Worker 并发领取任务，需要可靠的行锁或 `SKIP LOCKED`；
- 持续出现 SQLite 写锁等待或任务状态更新拥堵；
- 需要多租户、高可用、集中备份、远程数据库或更严格的审计权限。

迁移采用一次性停写窗口：备份 SQLite→执行 PostgreSQL 迁移→导入数据→校验表行数、关键关联和文件哈希→切换连接字符串。不要维护两套长期并行事实源。

```text
Nuxt 4 全栈应用
   ├── 管理面板：人物、参数、创作、反馈、历史
   ├── Nitro API：校验请求、创建任务、查询状态、审批发布
   └── 业务数据库：SQLite 默认 / PostgreSQL 升级
              │
              v
独立 Node Worker
   ├── 上下文适配器：本地 FTS5 / OpenViking
   ├── 文本/图片模型：生成适配器
...
所有运行返回 `runId`、`personaVersionId`、`modelVersion`、`promptVersion` 和 `contextProvider`，否则无法复现反馈前后的差异。
...
核心系统完成后，可以沿三条主线扩展：内容生产、内容评审和人物运行平台。按现有模块复用度与新增复杂度排序如下：

| 方向 | 业务价值 | 直接复用 | 必须新增 | 建议优先级 |
|---|---|---|---|---|
| IP 人物渠道化内容运营 | 为小说、动漫、游戏角色批量生成不同渠道内容 | 人物版本、图文生成、模板、审批、历史 | 内容日历、批量任务、渠道模板与发布接口 | P1 |
| 虚拟受众内容预评审 | 用多个人物独立评估文章、剧情、广告或选题，形成受众分布 | 兴趣判断、证据、参数快照、评测 | 批量人物运行、分群、聚合统计与对比报告 | P1 |
| 人物一致性 / OOC 审核 | 检查小说、剧本、对白是否违背人物设定并引用原著证据 | 人物档案、资料检索、rubric、运行历史 | 长文切分、逐段批注、冲突分类与审校报告 | P1 |
| 多人物编剧室 | 模拟不同人物对剧情事件的反应，辅助冲突和支线设计 | 多人物档案、兴趣与风格生成 | 世界状态、轮次编排、人物间共享事件和冲突消解 | P2 |
| 游戏 NPC / 互动角色接口 | 为游戏、互动小说提供持续角色对话与行为决策 | 人物版本、资料检索、反馈学习 | 会话状态、低延迟流式接口、并发控制和实时安全审核 | P2 |
| 品牌人格与数字员工 | 稳定生成品牌内容、销售话术或内部知识答复 | 人物版本、模板、审批、审计 | 知识时效、企业权限、合规规则和外部工具调用 | P2 |
| 人物模板/API 平台 | 向第三方提供人物创建、运行和评测能力 | 全部核心模块 | 多租户、配额、计费、密钥、隔离和服务等级 | P3 |
| 偏好数据与评测服务 | 输出人物偏好对、回归集和模型对比报告 | 反馈事件、A/B 选择、评测门禁 | 脱敏、数据授权、数据集版本和训练平台对接 | P3 |

推荐先验证三个方向：渠道化内容运营最容易形成直接产出；OOC 审核新增基础设施最少；虚拟受众预评审最能体现“模拟人物兴趣”的差异化。多人物实时互动、平台计费和模型训练都应等待明确客户需求。

当前只需稳定五个接缝：业务数据库、上下文适配器、模型适配器、文档渲染器和任务执行器。未来业务应通过这些接缝复用核心能力，不提前实现多租户、发布渠道、实时会话或计费。

### MVP 开发顺序

1. 定义人物、版本、资料、参数和运行状态机；验证：生效版本、参数来源和运行快照可追溯。
2. 建立管理面板骨架与上下文适配器；验证：关闭 OpenViking 时本地检索可运行，启用后可同步资料并查看状态。
...
- 不把 PostgreSQL 当作 SQLite 的在线热备，不做双写；
- 不把 OpenViking 设为系统运行的强制依赖；
...
8. SQLite 适合单机、少用户和单 Worker 的早期阶段；PostgreSQL 应作为达到并发与部署阈值后的迁移目标，而不是双写热备。（架构决策）
9. OpenViking 可以是可选语义检索适配器；只要业务库始终保留资料正文、人物版本和证据快照，关闭它不会破坏主流程。（架构决策）
