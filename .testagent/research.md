...
## 2026-08-30 灵魂保存即生效增量

### 验收清单

- 人物和世界列表移除设定状态列、待确认设定统计与相关说明，只保留启用状态。
- 新建人物和世界默认启用，初始提示词直接形成当前历史版本，不再形成待发布草稿。
- 灵魂详情直接显示当前提示词编辑框，不再通过弹窗、草稿或确认发布操作。
- 魔法棒只分析当前编辑文本并把结果回填编辑框，不自动保存；分析时显示可旋转的全屏模糊加载层。
- 历史图标显示全部保存版本；选择历史版本只回填编辑框，点击保存后才创建新的当前版本。
- 每次保存校验提示词预算，原子创建不可变历史版本并切换当前版本指针。
- 迁移把现有灵魂草稿转为最新当前版本并删除草稿，已有发布版本继续保留为历史。

### 目标与测试配对

- `SoulWorkspace.vue` 由 `tests/components/content-forms.test.ts` 覆盖编辑、AI 回填、历史回填和保存事件。
- 人物与世界列表页由 `tests/components/subject-list-creation.test.ts` 覆盖设定状态移除和创建请求。
- `SoulApplicationService.ts`、`ContentApplicationService.ts`、`SqliteContentRepository.ts` 由 `tests/integration/content-management.test.ts` 覆盖保存即生效和创建即生效。
- 数据迁移由 `tests/integration/sqlite-database.test.ts` 覆盖现有草稿提升、当前指针和外键完整性。
- 本轮静态配对扫描识别 235 个 TypeScript 源文件和 40 个测试文件；上述服务及仓储均已有测试配对。该结果是静态标识配对，不代表行或分支覆盖率。

### 测试缺口与断言审查

- 发现创建与直接保存路径缺少 Token 精确上限、超限无副作用测试，已补充同一用例同时验证边界成功、错误码、当前版本和版本总数。
- 发现直接保存路径缺少基础版本归属测试，已补充跨人物版本拒绝及当前版本不变断言。
- 实际注入两处 `>` 改为 `>=` 和一处移除归属校验的错误变体，3 个变体均被新增测试杀死；所有变体已经撤销。
- 本轮新增测试覆盖相等、异常、否定、状态副作用和深层结构断言，没有仅 `toBeDefined`、真值或自比较测试。
