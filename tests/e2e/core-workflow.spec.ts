...
test('首次设置、灵魂保存及文章直接生成形成可复现闭环', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'AI 蒸馏创建', exact: true }).click()
  await page.getByLabel('人物名称').fill('林默')
  await page.getByLabel('人物用途与聚焦方向').fill('严谨克制的学院观察员，关注课程、档案与古代文献，表达冷静简洁。')
  await page.getByRole('button', { name: '开始人物蒸馏', exact: true }).click()
  await page.getByRole('link', { name: '任务记录', exact: true }).click()
  const distillationHistoryLink = page.locator('tbody').getByRole('link', { name: '人物自由蒸馏', exact: true })
  await expect(distillationHistoryLink).toBeVisible()
  await expect(page.getByText('林默', { exact: true })).toBeVisible()
  await distillationHistoryLink.click()
  await expect(page).toHaveURL(/\/personas\/distillations\//)
  await expect(page.getByRole('heading', { name: '审阅分析并确认人物候选', exact: true })).toBeVisible({ timeout: 30_000 })
  await expect(page.getByText('人物分析报告', { exact: true })).toBeVisible()
  await expect(page.getByLabel('候选灵魂正文')).toHaveValue('严谨克制的学院观察员，关注课程、档案与古代文献，表达冷静简洁。')
  await page.getByRole('button', { name: '确认创建人物', exact: true }).click()
