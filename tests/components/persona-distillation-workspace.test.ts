...
describe('人物蒸馏后台工作区', () => {
  ...
  it('最终检查点展示证据，正文修改后只允许保存重评，未修改候选才允许确认', async () => {
    ...
    await wrapper.setProps({ run: createRun({ mode: 'update', status: 'awaiting_candidate_review' }) })
    expect(wrapper.text()).toContain('确认更新人物灵魂')
    expect(wrapper.text()).toContain('发布为当前人物新灵魂版本')
  })
