...
describe('世界与人物列表快速初始化', () => {
  ...
  it('人物与世界列表移除设定状态且默认每页十项并保留启停确认', async () => {
    ...
    const personaSummary = personaWrapper.findAll('.content-table-description')
      .find(description => description.text() === '可参与新任务。')
    if (!personaSummary) throw new Error('人物列表缺少灵魂介绍')
    expect(personaSummary.classes()).toContain('content-table-description--clamped')
    ...
    const worldSummary = worldWrapper.findAll('.content-table-description')
      .find(description => description.text() === '可进入新任务。')
    if (!worldSummary) throw new Error('世界列表缺少摘要')
    expect(worldSummary.classes()).toContain('content-table-description--clamped')
