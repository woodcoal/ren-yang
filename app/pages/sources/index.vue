...
<template>
  <div>
    <ContentPageHeader title="资料" description="SQLite 保存唯一正文，切片同步进入本地 FTS5；资料可复用于多个人物或世界。">
    ...
    <UCard class="mb-6">
      <template #header><h2 class="font-semibold text-highlighted">证据检索</h2></template>
      ...
      <div v-if="searchResults" class="mt-4 space-y-3">
        <p v-if="searchResults.length === 0" class="text-sm text-muted">没有匹配切片。</p>
        <div v-for="chunk in searchResults" :key="chunk.id" class="rounded-md border border-default p-3">
          <p class="text-xs font-medium text-primary">{{ chunk.heading || '无标题切片' }} · 第 {{ chunk.ordinal + 1 }} 段</p>
    ...
    <div v-else-if="sources.length" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <UCard v-for="source in sources" :key="source.id">
        ...
        <div class="mt-4 flex gap-4 text-xs text-muted"><span>{{ source.chunkCount }} 个切片</span><span>{{ source.linkCount }} 项关联</span><span>{{ source.inputType }}</span></div>
