...
const workflowSteps = [
  ...
  { label: '最终确认', description: '创建人物与灵魂' },
]
 ...
 * @returns 用户可理解的状态标题。
 */
function statusTitle(status: PersonaDistillationStatus): string {
  return ({
 ...
 */
function statusDescription(status: PersonaDistillationStatus): string {
  return ({
    ...
    completed: '人物及初始当前灵魂版本已原子写入，可以进入人物工作区继续维护。',
    failed: '固定输入和错误已经保留。可以基于相同快照创建一次新运行。',
    canceled: '运行停在安全点且没有创建人物；如需继续，请从人物列表重新创建。',
  })[status]
}
...
<template>
  <div>
    <ContentPageHeader
      ...
      :description="run ? statusDescription(run.status) : '读取可恢复的人物蒸馏运行。'"
    >
    ...
    <template v-else>
      ...
      <UCard v-if="active" class="run-status-board mt-6">
        <div class="flex items-start gap-4">
          ...
          <div>
            <h2 class="font-semibold text-highlighted">{{ statusTitle(run.status) }}</h2>
            <p class="mt-1 text-sm text-muted">{{ statusDescription(run.status) }}</p>
      ...
      <UCard v-else-if="run.status === 'failed'" class="mt-6">
        <UAlert color="error" :title="statusTitle(run.status)" :description="run.errorMessage || statusDescription(run.status)" />
      ...
      <UCard v-else-if="run.status === 'canceled'" class="mt-6">
        <UAlert color="neutral" :title="statusTitle(run.status)" :description="statusDescription(run.status)" />
      </UCard>

      <UCard v-else-if="run.status === 'completed'" class="mt-6">
        <UAlert color="success" :title="statusTitle(run.status)" :description="statusDescription(run.status)" />
        <div class="mt-4 flex justify-end">
          <UButton v-if="run.createdPersonaId" :to="`/personas/${run.createdPersonaId}`" icon="i-lucide-arrow-right">进入人物工作区</UButton>
        </div>
      </UCard>
    </template>
  </div>
