...
/** 当前批次排队或执行期间禁止再次提交同类提炼。 */
const isPending = computed(() => props.batch?.status === 'queued' || props.batch?.status === 'running')
...
<template>
  <UCard>
    <template #header>
      <div class="flex flex-wrap items-start justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-2">
          <UButton size="sm" :loading="loading" :disabled="loading || isPending" @click="emit('analyze', 'incremental')">结合新增素材提炼</UButton>
          <UButton size="sm" color="neutral" variant="soft" :loading="loading" :disabled="loading || isPending" @click="emit('analyze', 'full_rebuild')">从全部素材重新提炼</UButton>
    ...
    <template v-else>
      ...
      <p v-if="isPending" class="mt-3 text-sm text-muted">当前提炼完成前不能重复提交。</p>
