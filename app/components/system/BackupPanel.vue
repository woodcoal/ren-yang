...
async function createBackup(): Promise<void> {
  ...
  try {
    ...
    notifySuccess(`${response.data.backupId} · ${response.data.fileCount} 个文件 · ${formatBytes(response.data.totalBytes)}`, '备份创建完成')
  }
  catch (error: unknown) {
    notifyError(getApiErrorMessage(error, '备份创建失败'), '备份失败')
  }
  finally {
    loading.value = false
  }
