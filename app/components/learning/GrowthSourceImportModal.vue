...
const emptyGuidance = computed(() => props.subjectLabel === '人物'
  ? '当前没有可导入的人物反馈资料。请先在人物反馈资料中添加内容，再从这里批量评分导入。'
  : '当前没有可导入的世界资料。请先在“资料”标签中为这个世界加入资料，再从这里批量评分导入。')
...
<template>
  <UModal
    ...
    description="每份资料生成一条独立待确认成长；评分直接作为重要程度。确认启用后，成长会按相关性参加新任务。"
    scrollable
    :dismissible="!loading"
    :close="!loading"
        ...
        <div v-if="sources.length" class="space-y-3">
          ...
          >
            ...
            <UFormField label="评分" description="1–5 分" class="w-24 shrink-0">
              <UInput
                :model-value="scores[entry.source.id] ?? 3"
                type="number"
                min="1"
                max="5"
                :disabled="loading || !selectedIds.includes(entry.source.id)"
                @update:model-value="updateScore(entry.source.id, $event)"
              />
            </UFormField>
          </article>
        </div>
        <UAlert v-else color="neutral" title="暂时没有可导入资料" :description="emptyGuidance" />
