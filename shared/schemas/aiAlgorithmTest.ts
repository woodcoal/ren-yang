...
/** 成长或记忆提炼算法第一步的业务化测试输入。 */
export const growthExtractAlgorithmTestInputSchema = z.object({
  stepKey: z.literal('extract'),
  baselineText: z.string().trim().max(20_000, '当前学习提示词基线不能超过 20000 字'),
  materialText: z.string().trim().min(1, '提炼素材不能为空').max(100_000, '提炼素材不能超过 100000 字'),
}).strict()

/** 成长或记忆提炼算法第二步接收的第一步延续数据。 */
export const growthSynthesizeAlgorithmTestInputSchema = z.object({
  ...
  baselineJson: z.string().max(100_000, '学习提示词基线数据过长'),
  factsJson: z.string().max(500_000, '提炼事实数据过长'),
}).strict()
