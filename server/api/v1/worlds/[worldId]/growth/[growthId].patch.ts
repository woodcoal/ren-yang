...
/**
 * 修改世界成长素材的标题、正文快照和评分。
 * @param event 当前 H3 请求，路径包含世界和素材 UUID，请求体包含新内容。
 * @returns 修改后的世界成长工作区响应。
 */
async function handleUpdateWorldGrowth(event: H3Event) {
  return await executeController(event, async () => {
    ...
    return await event.context.applicationServices.learning.updateGrowthMaterial('world', worldId, growthId, input)
  })
}
