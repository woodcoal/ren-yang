...
/**
 * 批量永久删除世界成长及其全部历史修订。
 * @param event 当前 H3 请求，路径包含世界 UUID，请求体包含成长 UUID 集合。
 * @returns 原子删除后的最新世界成长工作区响应。
 */
async function handleDeleteWorldGrowth(event: H3Event) {
  return await executeController(event, async () => {
    ...
    return await event.context.applicationServices.learning.deleteGrowth('world', worldId, input)
  })
}
