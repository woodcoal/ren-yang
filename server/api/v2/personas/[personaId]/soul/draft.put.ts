...
import { saveSoulDraftSchema } from '#shared/schemas/content'
...
/**
 * 创建或覆盖人物当前唯一灵魂草稿。
 * @param event 已认证 API Key 且包含人物 UUID、用户名或邮箱与草稿的请求事件。
 * @returns 保存后的人物灵魂草稿。
...
export default defineEventHandler(async (event) => {
  ...
  return await executePublicWriteController(event, 'persona:write', {
    payload: body, targetType: 'persona_soul_draft', successStatusCode: 200, targetId: readPublicPersonaId,
  }, async () => {
    ...
    return toPublicJson(await event.context.applicationServices.soul.saveDraft('persona', personaId, saveSoulDraftSchema.parse(body)))
  })
})
