...
import { updatePersonaStatusSchema } from '#shared/schemas/content'
...
/**
 * 启用或停用指定人物。
 * @param event 已认证 API Key 且包含人物 UUID、用户名或邮箱与状态的请求事件。
 * @returns 状态更新后的人物详情。
...
export default defineEventHandler(async (event) => {
  ...
  return await executePublicWriteController(event, 'persona:write', {
    ...
    return toPublicJson(await event.context.applicationServices.content.updatePersonaStatus(personaId, updatePersonaStatusSchema.parse(body)))
  })
})
