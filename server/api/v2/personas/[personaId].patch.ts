...
/**
 * 修改人物名称和唯一世界关系。
 * @param event 已认证 API Key 且包含人物 UUID 与请求体的事件。
 * @returns 修改后的人物详情。
...
export default defineEventHandler(async (event) => {
  ...
  }, async () => toPublicJson(await event.context.applicationServices.content.updatePersona(
    resourceIdSchema.parse(rawId), updatePersonaSchema.parse(body),
  )))
})
