...
import type {
  PersonaAvatarFile,
  PersonaAvatarOriginalInput,
  PersonaAvatarStorage,
  PersonaAvatarVariant,
} from '../../ports/PersonaAvatarStorage'
...
/** 仅在模型二次裁剪时存在的裁剪前原图文件名。 */
const ORIGINAL_AVATAR_FILE_NAME = 'avatar-original'
...
export class LocalPersonaAvatarStorage implements PersonaAvatarStorage {
   ...
   */
  async hasAvatar(personaId: string, variant: PersonaAvatarVariant = 'result'): Promise<boolean> {
    const directory = this.resolvePersonaDirectory(personaId)
    try {
      await access(resolve(directory, avatarFileName(variant)))
      return true
    }
    catch (error: unknown) {
  ...
  async saveAvatar(
    ...
    const directory = this.resolvePersonaDirectory(personaId)
    const mediaType = validateAvatarFile(bytes, declaredMediaType)
    if (original) validateAvatarFile(original.bytes, original.declaredMediaType)
    await mkdir(directory, { recursive: true })
    const target = resolve(directory, AVATAR_FILE_NAME)
    const originalTarget = resolve(directory, ORIGINAL_AVATAR_FILE_NAME)
    const temporary = resolve(directory, `.${AVATAR_FILE_NAME}.${randomUUID()}.tmp`)
    const originalTemporary = resolve(directory, `.${ORIGINAL_AVATAR_FILE_NAME}.${randomUUID()}.tmp`)
    try {
      await this.capacity.assertCanWrite(directory, bytes.byteLength + (original?.bytes.byteLength ?? 0))
      await writeFile(temporary, bytes, { flag: 'wx' })
      if (original) await writeFile(originalTemporary, original.bytes, { flag: 'wx' })
      await rename(temporary, target)
      if (original) await rename(originalTemporary, originalTarget)
      else await rm(originalTarget, { force: true })
    }
    catch (error: unknown) {
      await Promise.all([
   ...
   */
  async readAvatar(personaId: string, variant: PersonaAvatarVariant = 'result'): Promise<PersonaAvatarFile> {
    const directory = this.resolvePersonaDirectory(personaId)
    const target = resolve(directory, avatarFileName(variant))
 ...
 * 返回指定头像变体的受控文件名。
 * @param variant 最终头像或二次裁剪前原图。
 * @returns 不含目录语义的固定文件名。
 */
function avatarFileName(variant: PersonaAvatarVariant): string {
  return variant === 'original' ? ORIGINAL_AVATAR_FILE_NAME : AVATAR_FILE_NAME
}
...
/**
 * 校验头像大小、魔数和声明类型。
 * @param bytes 待保存的头像字节。
 * @param declaredMediaType 浏览器或图片模型声明的媒体类型。
 * @returns 从文件魔数识别的可信媒体类型。
 */
function validateAvatarFile(bytes: Uint8Array, declaredMediaType: string | null): PersonaAvatarFile['mediaType'] {
  if (bytes.byteLength === 0 || bytes.byteLength > MAX_AVATAR_BYTES) {
