...
export class SwitchableContextProvider implements ContextProvider {
   ...
   * @param openVikingEnabled 当前动态配置中的明确开关或读取函数。
   */
  constructor(
    private readonly local: ContextProvider,
    private readonly openViking: ContextProvider & Pick<OpenVikingPort, 'checkHealth'>,
    private readonly openVikingEnabled: boolean | (() => boolean),
  ) {}

  /** @returns 当前数据库设置是否选择 OpenViking。 */
  private isOpenVikingEnabled(): boolean {
    return typeof this.openVikingEnabled === 'function' ? this.openVikingEnabled() : this.openVikingEnabled
  }

  /** @returns 新运行实际固定的提供器。 */
  getProvider(): 'sqlite_fts5' | 'openviking' {
    return this.isOpenVikingEnabled() ? 'openviking' : 'sqlite_fts5'
  }
  ...
  async search(request: EvidenceSearchRequest) {
    if (!this.isOpenVikingEnabled()) return await this.local.search(request)
    try {
