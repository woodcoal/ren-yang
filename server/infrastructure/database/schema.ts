...
export const personaDistillationRuns = sqliteTable(
  'persona_distillation_runs',
  {
    ...
    preparedPromptHash: text('prepared_prompt_hash'),
  ...
  table => [
    ...
    check('persona_distillation_runs_status_check', sql`${table.status} IN ('analyzing', 'awaiting_candidate_review', 'completed', 'failed', 'canceled')`),
    ...
    check('persona_distillation_runs_analysis_check', sql`${table.analysisReport} IS NULL OR length(trim(${table.analysisReport})) > 0`),
    ...
    check('persona_distillation_runs_prepared_hash_check', sql`${table.preparedPromptHash} IS NULL OR length(${table.preparedPromptHash}) = 64`),
